...
import { currentCompany, companyJobs, recentApplicants, workers, kanbanApplicants, jobAnalytics } from '../data/mock'
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px var(--kt-space-6)', display: 'flex', gap: 28, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{
            ...
            <Button variant="accent" size="md">Filter by Regulix Ready</Button>
          </div>
          ...
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', overflow: 'hidden' }}>
            <div style={{ padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--kt-border)' }}>
              <h2 style={{ fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', fontSize: 'var(--kt-text-md)' }}>Active Job Postings</h2>
              <Link to="/site/post-job" style={{ fontSize: 'var(--kt-text-sm)', color: 'var(--kt-accent)', textDecoration: 'none', fontWeight: 'var(--kt-weight-medium)' }}>
                + Add New
              </Link>
            </div>
            ...
            {/* Header row */}
            <div style={{
              display: 'grid', gridTemplateColumns: '2.2fr 1fr 1fr 1fr 80px 40px',
              padding: '10px 24px', borderBottom: '1px solid var(--kt-border)', background: 'var(--kt-bg)',
            }}>
                ...
                <React.Fragment key={job.id}>
                  ...
                  >
                    <div>
                      ...
                      {job.isSponsored && <Badge variant="accent" size="sm" style={{ marginTop: 4 }}>Featured</Badge>}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      ...
                      <span style={{
                        ...
                        color: job.regulixReadyApplicants > 0 ? 'var(--kt-olive-700)' : 'var(--kt-text)',
                        fontWeight: 'var(--kt-weight-medium)',
                      }}>
                        {job.totalApplicants}
                      </span>
                    </div>
                    <Badge variant={job.status === 'active' ? 'success' : 'secondary'} size="sm" dot>
                    ...
                    {analytics && (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 'var(--kt-radius-sm)', padding: '3px 7px',
          ...
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', overflow: 'hidden' }}>
            {/* Tab bar */}
            <div style={{ display: 'flex', borderBottom: '1px solid var(--kt-border)', padding: '0 24px' }}>
              {(['applicants', 'pipeline', 'saved'] as const).map(tab => (
                <button
                  ...
                  style={{
                    padding: '14px 16px',
                    fontSize: 'var(--kt-text-sm)', fontWeight: activeTab === tab ? 'var(--kt-weight-semibold)' : 'var(--kt-weight-normal)',
                ...
                >
                  {tab === 'applicants' ? 'Recent Applicants' : tab === 'pipeline' ? '🗂 Pipeline' : 'Saved Workers'}
                </button>
              ))}
            </div>
            ...
            <div style={{ padding: 24 }}>
              {activeTab === 'applicants' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {recentApplicants.map((worker, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'var(--kt-bg)', borderRadius: 'var(--kt-radius-md)', border: '1px solid var(--kt-border)' }}>
                      {/* Avatar */}
                      <div style={{
                        width: 40, height: 40, borderRadius: '50%',
                        background: 'var(--kt-navy-800)', color: 'var(--kt-sand-300)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontWeight: 'var(--kt-weight-bold)', fontSize: 'var(--kt-text-sm)', flexShrink: 0,
                        border: worker.isPremium ? '2px solid var(--kt-olive-400)' : '1px solid var(--kt-border)',
                      }}>
                        {worker.initials}
                      </div>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          <Link to={`/site/profile/${worker.id}`} style={{ textDecoration: 'none' }}>
                        ...
                        <p style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)', marginTop: 2 }}>
                          Applied for <strong>{(worker as typeof worker & { appliedJobTitle: string }).appliedJobTitle}</strong> · {(worker as typeof worker & { appliedDaysAgo: number }).appliedDaysAgo}d ago
                        </p>
                      </div>
              ...
              {activeTab === 'pipeline' && (
                <KanbanBoard initialApplicants={kanbanApplicants} />
              )}

              {activeTab === 'saved' && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
                  {savedWorkers.map(w => (
                    <WorkerCard key={w.id} worker={w} onViewProfile={() => { window.location.href = `/site/profile/${w.id}` }} />
                  ))}
                  {savedWorkers.length === 0 && (
                    <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px 0', color: 'var(--kt-text-muted)' }}>
        ...
        <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', padding: 18 }}>
            <h3 style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 12 }}>
              Company
            </h3>
            <p style={{ fontSize: 'var(--kt-text-sm)', color: 'var(--kt-text)', lineHeight: 1.6, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                ...
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--kt-text-xs)', gap: 8 }}>
                  <span style={{ color: 'var(--kt-text-muted)' }}>{row.label}</span>
                  <span style={{ color: 'var(--kt-text)', fontWeight: 'var(--kt-weight-medium)', textAlign: 'right' }}>{row.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', padding: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <Link to="/site/post-job" style={{ textDecoration: 'none' }}>
                <Button variant="accent" size="sm" style={{ width: '100%' }}>+ Post a Job</Button>
              </Link>
              <Link to="/site/jobs" style={{ textDecoration: 'none' }}>
                <Button variant="outline" size="sm" style={{ width: '100%' }}>Browse Worker Profiles</Button>
              </Link>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-olive-800)' }}>Regulix Tip</span>
            </div>
            <p style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)', lineHeight: 1.6 }}>
