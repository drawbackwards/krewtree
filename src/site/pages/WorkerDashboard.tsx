...
import { Badge, Button, Progress, Divider } from '../../components'
...
const statusConfig: Record<string, { variant: 'success' | 'info' | 'warning' | 'danger' | 'secondary'; label: string }> = {
...
const StarIcon = ({ filled }: { filled: boolean }) => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill={filled ? 'var(--kt-warning)' : 'none'} stroke={filled ? 'var(--kt-warning)' : 'var(--kt-border-strong)'} strokeWidth="2">
...
export const WorkerDashboard: React.FC = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--kt-bg)' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px var(--kt-space-6)', display: 'flex', gap: 28, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {!currentWorker.isRegulixReady && (
            <div style={{
              ...
              <Button variant="accent" size="md">Start Regulix →</Button>
            </div>
          )}

          {/* Applications Table */}
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', overflow: 'hidden' }}>
            <div style={{ padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--kt-border)' }}>
              <h2 style={{ fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', fontSize: 'var(--kt-text-md)' }}>My Applications</h2>
              <Link to="/site/jobs" style={{ fontSize: 'var(--kt-text-sm)', color: 'var(--kt-accent)', textDecoration: 'none', fontWeight: 'var(--kt-weight-medium)' }}>
                Browse More Jobs →
              </Link>
            </div>
            ...
            <div style={{
              ...
              padding: '10px 24px', borderBottom: '1px solid var(--kt-border)',
              background: 'var(--kt-bg)',
            }}>
              {['Job', 'Status', 'Applied', 'Actions'].map(h => (
                <span key={h} style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  {h}
                </span>
              ))}
            </div>
                ...
                <React.Fragment key={app.id}>
                  <div
                    style={{
                      ...
                      background: app.status === 'Offer' ? 'color-mix(in srgb, var(--kt-success) 5%, transparent)' : 'transparent',
                    }}
                  >
                    {/* Job info */}
                    <div>
                      ...
                      {app.isBoosted && (
                        <span style={{
                          ...
                          marginTop: 4, fontSize: 'var(--kt-text-xs)', color: 'var(--kt-olive-700)',
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      {app.status === 'Offer' && (
                        <Button variant="accent" size="sm" onClick={() => {}}>View Offer</Button>
                      )}
                      {!app.isBoosted && app.status !== 'Rejected' && app.status !== 'Offer' && (
                        <button
                          onClick={() => setBoostingId(app.id)}
                          style={{
                            fontSize: 'var(--kt-text-xs)', color: 'var(--kt-olive-700)',
                            background: boostingId === app.id ? 'var(--kt-olive-100)' : 'transparent',
                      ...
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 'var(--kt-radius-sm)', padding: '4px 8px',
                  ...
                  {isExpanded && (
                    <div style={{
                      ...
                      <p style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 12 }}>
                          ...
                          <div key={ev.id} style={{ display: 'flex', gap: 12, position: 'relative' }}>
                            ...
                            {ei < events.length - 1 && (
                              <div style={{
                                ...
                                width: 2, background: 'var(--kt-border)',
                              }} />
                            )}
                            <div style={{
                              width: 26, height: 26, borderRadius: '50%',
                              background: 'var(--kt-surface)', border: '2px solid var(--kt-border)',
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                            ...
                            <div style={{ paddingBottom: 14 }}>
                              <p style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', marginBottom: 1 }}>
                                {ev.status}
                              </p>
                              <p style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)' }}>
                                {ev.note} · {ev.occurredDaysAgo === 0 ? 'Today' : `${ev.occurredDaysAgo}d ago`}
                              </p>
                            </div>
                          </div>
                        ))}
          ...
          {recommendedJobs.length > 0 && (
            <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', overflow: 'hidden' }}>
              <div style={{ padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--kt-border)' }}>
                <h2 style={{ fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', fontSize: 'var(--kt-text-md)' }}>
                  ✨ Recommended for You
                </h2>
                <Link to="/site/jobs" style={{ fontSize: 'var(--kt-text-sm)', color: 'var(--kt-accent)', textDecoration: 'none', fontWeight: 'var(--kt-weight-medium)' }}>
                  See all →
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {recommendedJobs.map((job, i) => (
                  <Link key={job.id} to={`/site/jobs/${job.id}`} style={{
                    textDecoration: 'none',
                    display: 'flex', alignItems: 'center', gap: 14,
                    padding: '14px 24px',
                    borderBottom: i < recommendedJobs.length - 1 ? '1px solid var(--kt-border)' : 'none',
                  ...
                  >
                    <div style={{
                      ...
                      fontSize: 'var(--kt-text-md)', flexShrink: 0,
                    }}>
                      {job.company.name.charAt(0)}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 'var(--kt-text-sm)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', marginBottom: 2 }}>{job.title}</p>
                      <p style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)' }}>{job.company.name} · {job.location} · ${job.payMin}–${job.payMax}/hr</p>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
                      <Badge variant="secondary" size="sm">{job.type}</Badge>
                      {job.isSponsored && <Badge variant="accent" size="sm">Featured</Badge>}
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {currentWorker.performanceScore && (
            <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', padding: 24 }}>
              <h2 style={{ fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', fontSize: 'var(--kt-text-md)', marginBottom: 16 }}>
                Performance Score
              </h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{
                  ...
                  <span style={{ fontSize: 'var(--kt-text-xl)', fontWeight: 'var(--kt-weight-bold)', color: 'var(--kt-olive-700)' }}>
                    {currentWorker.performanceScore}
                  </span>
                </div>
                <div>
                  ...
                  <p style={{ fontSize: 'var(--kt-text-sm)', color: 'var(--kt-text-muted)' }}>
                    ...
                    {currentWorker.totalHoursWorked && ` · ${currentWorker.totalHoursWorked.toLocaleString()} hours worked`}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <h3 style={{ fontSize: 'var(--kt-text-sm)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)' }}>Profile Strength</h3>
              <span style={{ fontSize: 'var(--kt-text-sm)', fontWeight: 'var(--kt-weight-bold)', color: 'var(--kt-olive-700)' }}>
                {currentWorker.profileCompletePct}%
              </span>
            </div>
            <Progress
              value={currentWorker.profileCompletePct}
              color={currentWorker.profileCompletePct >= 90 ? 'success' : currentWorker.profileCompletePct >= 60 ? 'warning' : 'danger'}
              size="sm"
            ...
            <Link to={`/site/profile/${currentWorker.id}`} style={{ textDecoration: 'none', display: 'block' }}>
              <Button variant="outline" size="sm" style={{ width: '100%' }}>View My Profile</Button>
            </Link>
          </div>

          {/* Quick Links */}
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', padding: 20 }}>
            <h3 style={{ fontSize: 'var(--kt-text-sm)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', marginBottom: 12 }}>Quick Links</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                ...
                <Link
                  ...
                  style={{
                    ...
                    fontSize: 'var(--kt-text-sm)', color: 'var(--kt-text)',
                    textDecoration: 'none', fontWeight: 'var(--kt-weight-medium)',
                    background: 'var(--kt-bg)',
                  }}
                >
                  {link.label}
          ...
          {/* Skills */}
          <div style={{ background: 'var(--kt-surface)', border: '1px solid var(--kt-border)', borderRadius: 'var(--kt-radius-lg)', padding: 20 }}>
            <h3 style={{ fontSize: 'var(--kt-text-sm)', fontWeight: 'var(--kt-weight-semibold)', color: 'var(--kt-text)', marginBottom: 12 }}>My Top Skills</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {currentWorker.skills.map(skill => (
                <div key={skill.name}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-medium)', color: 'var(--kt-text)' }}>{skill.name}</span>
                    <span style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)' }}>{skill.level}</span>
                  </div>
                  <Progress
