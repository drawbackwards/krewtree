  ...
  return (
    <div className={styles.card} onClick={onViewProfile}>
      ...
      <div className={styles.body}>
        ...
        {appliedJobTitle && (
          <p style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)', marginBottom: 8 }}>
            Applied for <strong>{appliedJobTitle}</strong>
            {appliedDaysAgo !== undefined && ` · ${appliedDaysAgo === 0 ? 'today' : `${appliedDaysAgo}d ago`}`}
          </p>
        )}

        <div className={styles.skills}>
          {worker.skills.slice(0, 3).map(s => (
            <span key={s.name} className={styles.skill}>{s.name}</span>
          ))}
        </div>
      ...
      {showActions && (
        <div className={styles.actions} onClick={e => e.stopPropagation()}>
          <Button size="sm" variant="primary" onClick={onMessage}>Message</Button>
          <Button size="sm" variant="outline" onClick={onViewProfile}>View Profile</Button>
          {onFavorite && (
