...
const Swatch: React.FC<{ color: string; name: string; hex: string }> = ({ color, name, hex }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
    ...
    <span style={{ fontSize: 'var(--kt-text-xs)', fontWeight: 'var(--kt-weight-medium)', color: 'var(--kt-text)' }}>{name}</span>
    <span style={{ fontSize: 'var(--kt-text-xs)', color: 'var(--kt-text-muted)', fontFamily: 'var(--kt-font-mono)' }}>{hex}</span>
  </div>
)
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 'var(--kt-text-lg)', fontWeight: 'var(--kt-weight-bold)', color: 'var(--kt-text)' }}>
            Krewtree UI
          </span>
      ...
      {/* ---- Main ---- */}
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: 'var(--kt-space-10) var(--kt-space-8)' }}>
        ...
        <div style={{ marginBottom: '3rem', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 'var(--kt-text-lg)', color: 'var(--kt-text-muted)', maxWidth: 560, margin: '0 auto' }}>
            A full token-driven design system built from the Krewtree brand guidelines — Navy, Sand Dune &amp; Olive.
          </p>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <Row>
                ...
                <Button variant="accent" leftIcon={
        ...
        <Section title="Form Fields">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
            ...
            />
            <Textarea label="Bio" placeholder="Tell us about yourself..." maxChars={200} helperText="Max 200 characters" />
          </div>
          ...
          <div style={{ marginTop: 24, display: 'flex', flexWrap: 'wrap', gap: 24 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <Checkbox label="With hint" helperText="Optional extra context" checked={false} onChange={() => {}} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div>
              ...
              <AvatarGroup max={4} size="md">
                {['AK','BL','CM','DN','EO','FP'].map(i => (
                  <Avatar key={i} initials={i} variant={['primary','secondary','accent','neutral'][Math.floor(Math.random()*4)] as never} />
                ))}
              </AvatarGroup>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
              ...
              <Tabs activeTab={activeTab} onTabChange={setActiveTab}>
                ...
                <TabPanel id="overview">
                  <p style={{ color: 'var(--kt-text-muted)' }}>
                    This is the <strong>Overview</strong> panel. It shows summary information about the project.
                  </p>
                </TabPanel>
                <TabPanel id="members">
                  <p style={{ color: 'var(--kt-text-muted)' }}>
            ...
            <div>
              ...
              <Tabs defaultTab="tab1" variant="pill">
                ...
                <TabPanel id="tab1"><p style={{ color: 'var(--kt-text-muted)' }}>All items shown.</p></TabPanel>
                <TabPanel id="tab2"><p style={{ color: 'var(--kt-text-muted)' }}>Active items only.</p></TabPanel>
                <TabPanel id="tab3"><p style={{ color: 'var(--kt-text-muted)' }}>Archived items.</p></TabPanel>
              </Tabs>
            </div>
          </div>
        </Section>
        ...
        <Section title="Alerts">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Alert variant="info" title="Heads up!" description="Your account will reset in 3 days. Upgrade to keep your data." closable />
            <Alert variant="success" title="Payment received" description="Your transaction was processed successfully." closable />
            <Alert variant="warning" title="Storage almost full" description="You've used 90% of your 5GB storage." closable />
            <Alert variant="danger" title="Action required" description="Your subscription has expired. Please renew to continue." closable />
            <Alert variant="neutral" title="Maintenance window" description="Scheduled maintenance on Sunday at 2am UTC." />
          </div>
        </Section>
          ...
          <Modal
            ...
            footer={
              <>
                ...
                <Button variant="primary" onClick={() => setModalOpen(false)}>Create Project</Button>
              </>
            }
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Section title="Toast Notifications">
          <Row>
            <Button
              variant="primary"
              onClick={() => toast({ title: 'Project saved', description: 'Your changes have been saved.', variant: 'success' })}
            >
              Success Toast
            </Button>
            <Button
              variant="outline"
              onClick={() => toast({ title: 'Heads up', description: 'This might take a moment.', variant: 'info' })}
            >
              Info Toast
            </Button>
            <Button
              variant="ghost"
              onClick={() => toast({ title: 'Storage warning', description: 'Running low on storage.', variant: 'warning' })}
            >
              Warning Toast
            </Button>
            <Button
              variant="danger"
              onClick={() => toast({ title: 'Upload failed', description: 'Could not process your file.', variant: 'danger' })}
            >
              Error Toast
            </Button>
            <Button
        ...
        <Section title="Typography">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              { size: '15px', weight: 400, text: 'Body — Regular text for paragraphs and UI copy', token: 'text-md' },
              { size: '13px', weight: 400, text: 'Small — Helper text, captions, meta', token: 'text-sm' },
              { size: '11px', weight: 500, text: 'XSmall — Labels, badges, tags', token: 'text-xs' },
