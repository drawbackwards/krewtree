...
--   match_score         — 0-100, populated by trigger on insert (see migration 20260422000004)
...
-- Runs after the existing `set_applications_updated_at` trigger (alphabetical order).
...
DROP TRIGGER IF EXISTS stamp_applications_status_updated_at ON applications;
...
CREATE INDEX IF NOT EXISTS idx_applications_company_recent
