...
CREATE TABLE IF NOT EXISTS application_task (
  ...
  -- A task can only be in one terminal state at a time.
...
-- Companies can read tasks on their applications.
...
-- Companies can insert/update/delete tasks on their applications.
...
-- Covers all stage types including terminal so an employer can log a final observation.
...
-- Written by the server on every pipeline action (stage moves, task mutations,
-- trigger fires, etc.). Append-only — rows are never updated or deleted.
...
-- Companies can read the log for their applications.
...
-- Only the server (service role) inserts log rows. No direct INSERT for company users.
-- (Set up via supabase service role key in server-side calls.)
